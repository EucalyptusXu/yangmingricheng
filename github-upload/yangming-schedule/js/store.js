...
(function (global) {
  ...
  var DEFAULTS = {
    ...
    settings: {
      ...
      startedAt: null,
      demoLoaded: false,
      /* 早晨固定推送：每天某个时点单独送一条"今日一句"到通知栏。
       * 用户的真需求是"先打开 App 之前先看到一句对的"，比日程更轻。 */
      morningPush: true,
      morningPushTime: '07:30'
    },
    feedback: {},
    fired: {}
  };
  ...
  /* ---------------- 早晨固定推送：未来 30 天的"今日一句" ----------------
   * 输入：quoteProvider(todayItems, dateStr, opts) -> {text, src}（由匹配引擎提供）
   * 关闭推送（morningPush=false）时返回空数组。
   * 触发时刻早于当前时间的那一天不排（已经过了就别补）。
   * 每天都生成一条；排程交给原生侧按 requestCode 整批撤换。
   */
  function collectMorningPush(quoteProvider, lookback) {
    var s = load();
    var st = s.settings;
    if (st.morningPush === false) return [];
    var hhmm = /^([01]\d|2[0-3]):[0-5]\d$/.test(st.morningPushTime || '') ? st.morningPushTime : '07:30';
    var parts = hhmm.split(':');
    var horizon = lookback || 30;
    var arr = [];
    var start = today();
    for (var i = 0; i < horizon; i++) {
      var d = shift(start, i);
      var trigger = new Date(d + 'T' + parts[0] + ':' + parts[1] + ':00').getTime();
      if (trigger <= Date.now()) continue;
      var pick = null;
      try {
        if (typeof quoteProvider === 'function') {
          pick = quoteProvider(byDate(d), d);
        }
      } catch (e) { pick = null; }
      if (!pick || !pick.text) continue;
      arr.push({
        id: 'morning_' + d,
        date: d,
        time: hhmm,
  ...
  global.YM_STORE = {
    ...
    collectMorningPush: collectMorningPush,
